 ...
 * as they arrive.
 * @dev Контракт является базовым для сбора средств за токены.
 TODO:
...
contract Crowdsale is Ownable {
    ...
    uint256 totalTokenAmount = SafeMath.mul(100000000,10 ** 18);
    ...
    */
    ...
    function buyTokens(address beneficiary) payable {
        // другой принцип продажи токенов (без эмиссии, продажа только суммы токенов, расположенной на кошельке владельца)
        ...
        require(tokenSold.add(tokenAmount) <= totalTokenAmount);
        // увеличить общее количество проданных токенов
        tokenSold.add(tokenAmount);
        // если получатель токенов - реферрал
        uint256 tokenAmountToAffiliate;
        if (Helpers.addressArrayContains(invitedReferrals, beneficiary)) {
            // подсчет вознаграждения аффилиата (приглашающего) в токенах (5%)
            tokenAmountToAffiliate = tokenAmount.mul(SafeMath.div(1,20));
            // проверка того, что на фондовом кошеле есть требуемое количество токенов
            require(token.balanceOf(token) >= tokenAmount.add(tokenAmountToAffiliate));
        ...
        } else { // если получатель токенов - инвестор
            // проверка того, что на фондовом кошеле есть требуемое количество токенов
            require(token.balanceOf(token) >= tokenAmount);
            // обновление счетчика присланных денег (капитализации)
            weiRaised = weiRaised.add(weiAmount);
            // перевод токенов с фондового кошелька на кошель инвестора
            token.transfer(beneficiary, tokenAmountToAffiliate);
        }
